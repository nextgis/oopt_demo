...
(function() {
  var build_gallery, build_info, build_pups, build_video, build_web, circleGeometry, cities, close_menu, close_popup, ellipsoid, fly_to_home, geodata, get_oopt_rect, handler, load_borders, load_cities, load_fz, load_geodata, load_np, load_popups_data, load_zp, num_images, oopt, open_menu, open_popup, pole_primitive, popups_data, prepare_popup, primitives, redCircleInstance, scene, selected_polygon_name, settings, showed_image, viewer;
  ...
  build_info = function(_id) {
    ...
    return $('.popup__panel--info iframe').load(function() {
      var head;
  ...
  build_video = function(_id) {
    var is_video_enable, video_parent;
    ...
    return $("video").on("error", function() {
      if ($('video').attr('src') === $('video').attr('src-mp4')) {
        ...
        return $('[data-target = video]').addClass("disabled");
      } else {
